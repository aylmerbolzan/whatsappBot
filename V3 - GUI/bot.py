...
def abrir_tela_cadastro(editar=False, destinatario_id=None):
    ...
    if editar and destinatario_id:
        conn = sqlite3.connect('destinatarios.db')
        c = conn.cursor()
        c.execute("SELECT nome, telefone, observacao FROM destinatarios WHERE id = ?", (destinatario_id,))
        destinatario = c.fetchone()
        conn.close()
        if destinatario:
            entry_nome.insert(0, destinatario[0])
            entry_telefone.insert(0, destinatario[1])
            text_observacao.insert("1.0", destinatario[2])
    ...
    def salvar_destinatario():
        ...
        if nome and telefone:
            ...
            c = conn.cursor()
            if editar and destinatario_id:
                c.execute("UPDATE destinatarios SET nome = ?, telefone = ?, observacao = ? WHERE id = ?",
                          (nome, telefone, observacao, destinatario_id))
            else:
                c.execute("INSERT INTO destinatarios (nome, telefone, observacao) VALUES (?, ?, ?)",
...
def carregar_destinatarios():
    ...
    conn = sqlite3.connect('destinatarios.db')
    c = conn.cursor()
    c.execute("SELECT id, nome, telefone, observacao FROM destinatarios")
...
def enviar_mensagens():
    ...
    for destinatario in selecionados:
        id_destinatario, nome, telefone, observacao = destinatario
        mensagem_final = mensagem_principal.replace("{nome}", nome)
        if observacao:
...
tree = ttk.Treeview(frame_listagem, columns=("ID", "Nome", "Telefone", "Observação"), show="headings")
...
button_cadastrar = ctk.CTkButton(frame_botoes, text="Cadastrar", command=abrir_tela_cadastro)
button_cadastrar.pack(side="left", padx=5)

button_editar = ctk.CTkButton(frame_botoes, text="Editar", command=editar_destinatario)
button_editar.pack(side="left", padx=5)

button_excluir = ctk.CTkButton(frame_botoes, text="Excluir", command=excluir_destinatario)
